...
private fun stopPlaybackInternal(showToast: Boolean = true) {
        ...
        return try {
            ...
            if (exactTarget != null) {
                ...
                if (safeBounds(exactTarget, b) && safeVisible(exactTarget) && safeEnabled(exactTarget)) {
                    ...
                    if (exactScore >= 110) return SmartMatch(exactTarget, Rect(b), 10000 + exactScore)
                }
            }

            findExactActionButtonAcrossWindows(gesture)?.let { exact ->
                if (exact.score >= 180) return exact
            }
            ...
            var visited = 0
            while (!stack.isEmpty() && visited < 1300) {
                visited++
                val node = stack.removeLast()
                val bounds = Rect()
                ...
                if (safeBounds(node, bounds)) {
                    ...
                    if (safeBounds(target, tb) && safeVisible(target) && safeEnabled(target)) {
                        var score = identityConfidenceForNode(target, tb, gesture)
                        if (safeClickable(target)) score += 12
    ...
    private fun scoreNode(node: AccessibilityNodeInfo, bounds: Rect, gesture: RecordedGesture): Int {
        return try {
            val target = findClickableParent(node) ?: node
            val tb = Rect()
            val finalBounds = if (safeBounds(target, tb)) tb else bounds
            identityConfidenceForNode(target, finalBounds, gesture)
        } catch (_: Exception) {
            0
        }
    }

    private fun captureTargetSnapshotInternal(x: Int, y: Int, screenW: Float, screenH: Float): TargetSnapshot? {
        return try {
