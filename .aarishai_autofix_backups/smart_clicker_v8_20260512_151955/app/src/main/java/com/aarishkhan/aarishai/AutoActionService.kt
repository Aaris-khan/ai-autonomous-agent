...
private fun stopPlaybackInternal(showToast: Boolean = true) {
    ...
    private fun geometryMatchThreshold(gesture: RecordedGesture): Int {
        ...
        if (hasStrongSavedIdentity(gesture)) return 70
        return when {
    ...
    private fun aarishCollectTextSeedBounds(
        ...
        for (root in roots) {
            for (seed in seeds) {
                try {
                    val matches = root.findAccessibilityNodeInfosByText(seed).orEmpty()
                    for (node in matches.take(40)) {
                        val action = findClickableParent(node) ?: node
                        val b = Rect()
    ...
    private fun smartMatchThreshold(match: SmartMatch, gesture: RecordedGesture): Int {
        if (aarishExactOrVeryStrong(match.node, gesture)) return 54
        ...
        if (confidence >= 0.90f || primarySim >= 0.94f) return 58
        if (confidence >= 0.80f || primarySim >= 0.84f) return 66
        if (confidence >= 0.70f || primarySim >= 0.74f) return 76

        if (!hasStrongSavedIdentity(gesture)) return geometryMatchThreshold(gesture)
        if (hasPrimary) return 82
    ...
    private fun performExactSmartTap(match: SmartMatch, gesture: RecordedGesture, runId: Int): Boolean {
        if (!isSamePlaybackRun(runId)) return false
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.N) return false
        if (match.bounds.width() <= 0 || match.bounds.height() <= 0) return false
        ...
        val safeX = (match.bounds.left + (gesture.insideXPercent.coerceIn(0f, 1f) * match.bounds.width()))
            .coerceIn(2f, screenW)
        val safeY = (match.bounds.top + (gesture.insideYPercent.coerceIn(0f, 1f) * match.bounds.height()))
            .coerceIn(2f, screenH)
    ...
    private fun performSmartNodeClick(match: SmartMatch, gesture: RecordedGesture, runId: Int): Boolean {
        ...
        val trusted =
            match.score >= smartMatchThreshold(match, gesture) ||
                aarishExactOrVeryStrong(actionNode, gesture) ||
                smartIdentityConfidence(actionNode, gesture) >= 0.74f ||
                isGeometryTrusted(match, gesture) ||
                (match.score >= 78 && hasStrongSavedIdentity(gesture))

        if (!trusted) return false

        if (performExactSmartTap(match, gesture, runId)) {
        ...
        if (!clicked) {
            ...
            while (!clicked && parent != null && depth < 10) {
                if (aarishExactOrVeryStrong(parent, gesture) || safeClickable(parent)) {
        ...
        handler.postDelayed(doneTask, 420L)
