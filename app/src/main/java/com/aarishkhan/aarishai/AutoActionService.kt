...
private fun stopPlaybackInternal(showToast: Boolean = true) {
    ...
    private fun scoreNode(
        ...
        return try {
            ...
            if (!gesture.targetId.isNullOrBlank() && nodeId == gesture.targetId) score += 150
            if (exactTextMatch) score += 120
            if (exactDescMatch) score += 120
            ...
            if (gesture.xPercent > 0f && gesture.yPercent > 0f) {
                ...
                else if (dx > 0.40f || dy > 0.40f) score -= if (strongIdentity) 0 else 45
            }
            ...
            if (!hasAnyIdentity) 0 else score.coerceAtLeast(0)
        } catch (_: Exception) {
            0
        }
    }
