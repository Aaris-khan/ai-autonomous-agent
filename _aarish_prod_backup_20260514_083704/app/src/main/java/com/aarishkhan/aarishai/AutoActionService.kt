...
private fun stopPlaybackInternal(showToast: Boolean = true) {
    ...
    private fun safeNodeChildCount(node: AccessibilityNodeInfo?): Int = safeChildCount(node)
    private fun safeNodeChild(node: AccessibilityNodeInfo?, index: Int): AccessibilityNodeInfo? = safeChild(node, index)
...
private fun getRealAppRootForPoint(tapX: Int?, tapY: Int?): AccessibilityNodeInfo? {
    ...
    private fun findDeepestNodeAtCoordinate(root: AccessibilityNodeInfo?, x: Int, y: Int): AccessibilityNodeInfo? {
        if (root == null) return null
        val stack = java.util.ArrayDeque<Pair<AccessibilityNodeInfo, Int>>()
        ...
        while (!stack.isEmpty() && visited < 1400) {
            ...
            if (bounds.contains(x, y)) {
                ...
                val candidateScore = depth * 1000 + (if (safeClickable(node)) 800 else 0) + (if (safeChildCount(node) == 0) 300 else 0) + (500000 / area)
...
private fun findExactActionButtonAcrossWindows(gesture: RecordedGesture): SmartMatch? {
    ...
    private inline fun <T> safeNodeRead(defaultValue: T, block: () -> T): T {
        return try { block() } catch (_: IllegalStateException) { defaultValue } catch (_: Exception) { defaultValue }
    }

    private fun findBestSmartTarget(gesture: RecordedGesture): SmartMatch? {
        return try {
            val point = aarishGesturePoint(gesture)
            val fallbackX = point.first
            val fallbackY = point.second

            val rootAtPoint = getRealAppRootForPoint(fallbackX.toInt(), fallbackY.toInt())
            val exactNode = findDeepestNodeAtCoordinate(rootAtPoint, fallbackX.toInt(), fallbackY.toInt())
            val exactTarget = findClickableParent(exactNode) ?: exactNode
            if (exactTarget != null) {
                val b = Rect()
                if (safeBounds(exactTarget, b) && safeVisible(exactTarget) && safeEnabled(exactTarget)) {
                    val exactScore = identityConfidenceForNode(exactTarget, b, gesture)
                    if (exactScore >= 110) return SmartMatch(exactTarget, Rect(b), 10_000 + exactScore)
                }
            }

            findExactActionButtonAcrossWindows(gesture)?.let { exactAction ->
                if (exactAction.score >= 180) return exactAction
            }

            val root = rootAtPoint ?: getRealAppRoot() ?: return null
            var best: SmartMatch? = null
            val stack = java.util.ArrayDeque<AccessibilityNodeInfo>()
            stack.add(root)
            var visitedNodes = 0

            while (!stack.isEmpty() && visitedNodes < 1300) {
                visitedNodes++
                val node = stack.removeLast()
                val bounds = Rect()
                if (safeBounds(node, bounds)) {
                    val targetNode = findClickableParent(node) ?: node
                    val targetBounds = Rect()
                    if (safeBounds(targetNode, targetBounds) && safeVisible(targetNode) && safeEnabled(targetNode)) {
                        var score = identityConfidenceForNode(targetNode, targetBounds, gesture)
                        if (safeClickable(targetNode)) score += 12
                        if (score > 0 && (best == null || score > best!!.score)) {
                            best = SmartMatch(targetNode, Rect(targetBounds), score)
                        }
                    }
                }
                val count = safeChildCount(node)
            ...
            val finalBest = best ?: return null
            if (finalBest.score >= 125) finalBest else null
        } catch (_: Exception) {
            null
        }
    }
    ...
    private fun scoreNode(node: AccessibilityNodeInfo, bounds: Rect, gesture: RecordedGesture): Int {
        return try {
            val targetNode = findClickableParent(node) ?: node
            val targetBounds = Rect()
            val finalBounds = if (safeBounds(targetNode, targetBounds)) targetBounds else bounds
            identityConfidenceForNode(targetNode, finalBounds, gesture)
        } catch (_: Exception) {
            0
        }
    }
...
private fun shouldUseSelectionLongPress(match: SmartMatch?, gesture: RecordedGesture): Boolean {
    ...
    private fun hasRealMovement(points: List<GesturePoint>): Boolean {
        ...
        for (p in points) {
            maxDx = max(maxDx, abs(p.x - first.x))
            maxDy = max(maxDy, abs(p.y - first.y))
        }
...
private fun performGestureAt(
    ...
    private fun captureTargetSnapshotInternal(
        ...
        return try {
            ...
            TargetSnapshot(
                ...
                insideXPercent = if (bounds.width() > 0) {
                    ((x - bounds.left).toFloat() / bounds.width()).coerceIn(0f, 1f)
                } else 0.5f,
                insideYPercent = if (bounds.height() > 0) {
                    ((y - bounds.top).toFloat() / bounds.height()).coerceIn(0f, 1f)
                } else 0.5f
            )
        } catch (_: Exception) {
            null
    ...
    private fun dispatchHybridGesturePath(
        ...
        fun dispatchBuilder(builder: GestureDescription.Builder, done: (Boolean) -> Unit) {
            val gesture = try { builder.build() } catch (_: Exception) {
                done(false)
                return
            }
        ...
        if (!hasLine) path.lineTo((safeX + 1f).coerceIn(2f, screenW), (safeY + 1f).coerceIn(2f, screenH))
