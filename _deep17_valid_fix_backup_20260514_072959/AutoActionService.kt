...
private fun getRealAppRootForPoint(tapX: Int?, tapY: Int?): AccessibilityNodeInfo? {
    ...
    private fun findDeepestNodeAtCoordinate(
        ...
        while (!stack.isEmpty()) {
            ...
            val b = Rect()
            if (!safeBounds(node, b)) continue

            if (b.contains(x, y)) {
                val area = (b.width() * b.height()).coerceAtLeast(1)
                val score = depth * 1200 +
                    if (safeClickable(node)) 900 else 0 +
                    if (safeChildCount(node) == 0) 350 else 0 +
                    800000 / area
                ...
                val count = safeChildCount(node)
                for (i in 0 until count) safeChild(node, i)?.let { stack.add(Pair(it, depth + 1)) }
            }
        }
...
private fun findExactActionButtonAcrossWindows(gesture: RecordedGesture): SmartMatch? {
    ...
    private fun findBestSmartTarget(gesture: RecordedGesture): SmartMatch? {
        return zeroWrongFindTarget(gesture)
    }
    ...
    private fun captureTargetSnapshotInternal(
        ...
        return try {
            ...
            val root = roots.firstOrNull { r ->
                val rb = Rect()
                safeBounds(r, rb) && rb.contains(x, y)
            } ?: roots.firstOrNull() ?: return null

            val touched = findDeepestNodeAtCoordinate(root, x, y) ?: return null
            val click = findClickableParent(touched) ?: touched

            val b = Rect()
            if (!safeBounds(click, b) || b.width() <= 0 || b.height() <= 0) return null

            val sw = screenW.coerceAtLeast(1f)
            val sh = screenH.coerceAtLeast(1f)
            val parent = safeParent(click)

            TargetSnapshot(
                targetText = safeText(click) ?: safeText(touched),
                targetDesc = safeDesc(click) ?: safeDesc(touched),
                targetId = safeId(click) ?: safeId(touched),
                targetClass = safeClass(click) ?: safeClass(touched),

                targetContextText = zSubtree(click, 60, 700),
                targetChildText = zSubtree(touched, 40, 500),
                targetSiblingText = zParentText(click),
                targetRoleFlags = listOf(
                    if (safeClickable(click)) "click" else "",
                    if (zRead(false) { click.isLongClickable }) "long" else "",
                    if (zRead(false) { click.isScrollable }) "scroll" else "",
                    if (zRead(false) { click.isEditable }) "edit" else "",
                    if (zRead(false) { click.isCheckable }) "check" else ""
                ).filter { it.isNotBlank() }.joinToString("|"),
                targetTreePath = zStableKey(click),

                targetParentText = zParentText(click),
                targetParentId = safeId(parent),
                targetParentClass = safeClass(parent),
                targetStableKey = zStableKey(click),
                targetNodeIndex = zIndexInParent(click),
                zeroWrongTap = true,
                ...
                xPercent = (x / sw).coerceIn(0f, 1f),
                yPercent = (y / sh).coerceIn(0f, 1f),
                targetWPercent = (b.width() / sw).coerceIn(0f, 1f),
                targetHPercent = (b.height() / sh).coerceIn(0f, 1f),
                insideXPercent = ((x - b.left).toFloat() / b.width().coerceAtLeast(1)).coerceIn(0f, 1f),
                insideYPercent = ((y - b.top).toFloat() / b.height().coerceAtLeast(1)).coerceIn(0f, 1f)
            )
        } catch (_: Exception) {
            null
        }
