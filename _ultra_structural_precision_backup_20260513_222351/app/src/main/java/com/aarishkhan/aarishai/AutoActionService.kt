...
private fun normalizeUltraText(value: String?): String {
    ...
    private fun findExactActionButtonAcrossWindows(
        ...
        return supremeFindAtOriginalPoint(gesture)
    }
    ...
    private fun findBestSmartTarget(gesture: RecordedGesture): SmartMatch? {
        return try {
            supremeFindAtOriginalPoint(gesture)?.let { return it }
            ...
            while (!stack.isEmpty() && visited < 900) {
                ...
                val action = supremeClickableParent(node) ?: node
                val b = Rect()
                if (supremeBounds(action, b)) {
                    val score = supremeScoreFor(action, b, gesture)
                    if (score >= SUPREME_ACCEPT_SCORE && (best == null || score > best!!.score)) {
                ...
                val c = supremeChildCount(node)
                for (i in 0 until c) {
                    supremeChild(node, i)?.let { stack.add(it) }
                }
            }

            best?.takeIf { it.score >= SUPREME_ACCEPT_SCORE }
        } catch (_: Exception) {
            null
        }
    }
    ...
    private fun scoreNode(
        ...
        return try {
            supremeScoreFor(node, bounds, gesture)
        } catch (_: Exception) {
            SUPREME_REJECT_SCORE
        }
    }
    ...
    private fun smartIdentityConfidence(
        ...
        return try {
            val action = supremeClickableParent(node) ?: node ?: return 0f
            val b = Rect()
            if (!supremeBounds(action, b)) return 0f
            val score = supremeScoreFor(action, b, gesture)
            if (score >= SUPREME_ACCEPT_SCORE) 1f else 0f
        } catch (_: Exception) {
            0f
        }
    }
    ...
    private fun isAmbiguousSmartMatch(
        ...
        val bestDrift = supremeSpatialDrift(
            best.bounds,
            gesture,
        ...
        val runnerDrift = supremeSpatialDrift(
            runnerUp.bounds,
            gesture,
            resources.displayMetrics.widthPixels.toFloat().coerceAtLeast(1f),
            resources.displayMetrics.heightPixels.toFloat().coerceAtLeast(1f)
        ...
        if (!exactIdentityHit(best.node, gesture)) return true
        if (runnerUp.score >= best.score - 1) return true
        if (runnerDrift + 0.01f < bestDrift) return true
    ...
    private fun exactIdentityHit(node: AccessibilityNodeInfo?, gesture: RecordedGesture): Boolean {
        return try {
            val action = supremeClickableParent(node) ?: node ?: return false
            val b = Rect()
            if (!supremeBounds(action, b)) return false
            supremeScoreFor(action, b, gesture) >= SUPREME_ACCEPT_SCORE
        } catch (_: Exception) {
            false
        }
    }
