...
private fun stopPlaybackInternal(showToast: Boolean = true) {
    ...
    private fun safeNodeVisible(node: AccessibilityNodeInfo?) = safeVisible(node)
    private fun safeNodeEnabled(node: AccessibilityNodeInfo?) = safeEnabled(node)
    private fun safeNodeClickable(node: AccessibilityNodeInfo?) = safeClickable(node)
    private fun safeNodeChildCount(node: AccessibilityNodeInfo?) = safeChildCount(node)
    private fun safeNodeChild(node: AccessibilityNodeInfo?, index: Int) = safeChild(node, index)
    private fun safeNodeParent(node: AccessibilityNodeInfo?) = safeParent(node)
    private fun safeNodeBounds(node: AccessibilityNodeInfo?, out: Rect) = safeBounds(node, out)
    private fun safeNodeText(node: AccessibilityNodeInfo?) = safeText(node)
    private fun safeNodeDesc(node: AccessibilityNodeInfo?) = safeDesc(node)
    private fun safeNodeId(node: AccessibilityNodeInfo?) = safeId(node)
    private fun safeNodeClass(node: AccessibilityNodeInfo?) = safeClass(node)
...
private fun identityConfidenceForNode(
    node: AccessibilityNodeInfo,
    bounds: Rect,
    ...
    if (!exactText && !gesture.targetText.isNullOrBlank() &&
        nodeText?.contains(gesture.targetText, ignoreCase = true) == true
    ) score += 60

    if (!exactDesc && !gesture.targetDesc.isNullOrBlank() &&
        ...
        return try {
            ...
            if (exactTarget != null) {
                ...
                if (safeBounds(exactTarget, b) && safeVisible(exactTarget) && safeEnabled(exactTarget)) {
                    ...
                    if (exactScore >= 110) {
                        return SmartMatch(exactTarget, Rect(b), 10_000 + exactScore)
                    }
                }
            }
            ...
            return try {
                ...
                for (root in roots) {
                    ...
                    while (!stack.isEmpty() && visited < 1600) {
                        ...
                        if (safeVisible(node) && (safeClickable(node) || safeFocusable(node)) && markers.any { label.contains(it.lowercase()) }) {
                            return true
                        }
