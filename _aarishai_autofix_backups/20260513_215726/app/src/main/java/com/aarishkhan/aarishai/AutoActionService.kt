...
private fun normalizeUltraText(value: String?): String {
    ...
    private fun roleFlagsOf(node: AccessibilityNodeInfo?): String {
        ...
        if (safeRead(false) { node.isClickable }) flags.add("click")
        if (safeRead(false) { node.isLongClickable }) flags.add("long")
        if (safeRead(false) { node.isEditable }) flags.add("edit")
        if (safeRead(false) { node.isScrollable }) flags.add("scroll")
        if (safeRead(false) { node.isCheckable }) flags.add("check")
        if (safeRead(false) { node.isChecked }) flags.add("checked")
        if (safeRead(false) { node.isEnabled }) flags.add("enabled")
        if (safeRead(false) { node.isVisibleToUser }) flags.add("visible")
        if (safeRead(false) { node.isFocusable }) flags.add("focus")
...
private fun captureTargetSnapshotInternal(
    ...
    private fun dispatchHybridGesturePath(
        ...
        val holdDuration = duration.coerceAtLeast(if (isDoubleTap) 220L else 60L).coerceAtMost(3500L)
        ...
        if (isDoubleTap) {
            val firstTap = GestureDescription.Builder().apply {
                addStroke(GestureDescription.StrokeDescription(tinyPath(), 0L, 70L))
            }
        ...
        val normalBuilder = GestureDescription.Builder().apply {
            addStroke(GestureDescription.StrokeDescription(path, 0L, duration.coerceAtLeast(60L).coerceAtMost(3500L)))
        }
