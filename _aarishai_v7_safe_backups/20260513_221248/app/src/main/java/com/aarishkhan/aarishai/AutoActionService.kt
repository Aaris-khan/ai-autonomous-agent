    ...
    private fun findExactActionButtonAcrossWindows(
        ...
        return try {
            ...
            if (activeRoot != null && !skipRoot(activeRoot)) {
                val b = Rect()
                if (safeBounds(activeRoot, b) && b.width() > 0 && b.height() > 0) roots.add(activeRoot)
            }
            ...
            for (root in roots) {
                ...
                while (!stack.isEmpty() && visited < 1200) {
                    ...
                    if (text == wantedLower || desc == wantedLower) {
...
private fun captureTargetSnapshotInternal(
    ...
    private fun dispatchHybridGesturePath(
        ...
        val movedPx = aarishV6MovementPx(points)
        val isSwipe = points.size > 1 && movedPx > 40f
        val normalDuration = when {
            isSwipe -> duration.coerceAtLeast(120L).coerceAtMost(5000L)
            isDoubleTap -> duration.coerceAtLeast(220L).coerceAtMost(1500L)
            else -> duration.coerceAtLeast(55L).coerceAtMost(1200L)
        }
        ...
        fun dispatchPath(path: android.graphics.Path, strokeMs: Long, done: (Boolean) -> Unit) {
            if (!isSamePlaybackRun(runId)) {
                done(false)
                return
            }
            ...
            val gesture = try {
                android.accessibilityservice.GestureDescription.Builder()
                    .addStroke(android.accessibilityservice.GestureDescription.StrokeDescription(path, 0L, strokeMs))
                    .build()
            } catch (_: Exception) {
                done(false)
                return
            ...
            val accepted = dispatchGesture(
                ...
                object : android.accessibilityservice.AccessibilityService.GestureResultCallback() {
                    override fun onCompleted(gestureDescription: android.accessibilityservice.GestureDescription?) {
                        ...
                        if (isSamePlaybackRun(runId)) {
                            aarishV6HapticTick()
                            GestureStore.aarishV6RecordClick(1)
                            done(true)
                        } else {
                            done(false)
                        }
        ...
        fun buildReplayPath(): android.graphics.Path {
            val path = android.graphics.Path().apply { moveTo(safeX, safeY) }
            var hasLine = false
        ...
        if (isDoubleTap) {
            val firstTap = aarishV6TapPath(safeX, safeY, screenW, screenH)
            dispatchPath(firstTap, 75L) { firstOk ->
                if (!firstOk || !isSamePlaybackRun(runId)) {
                    ...
                    handler.postDelayed({
                        if (!isSamePlaybackRun(runId)) {
                            ...
                            val secondTap = aarishV6TapPath(safeX, safeY, screenW, screenH)
                            dispatchPath(secondTap, normalDuration) { secondOk ->
                                finishOnce(secondOk && isSamePlaybackRun(runId))
                            }
                        }
                    }, GestureStore.aarishV6HumaniseDelay(this, 120L))
                }
            }
            return
        }

        val burstCount = if (isSwipe) 1 else GestureStore.aarishV6GetMultiTapCount(this).coerceIn(1, 10)
        if (burstCount > 1) {
            fun runBurst(index: Int) {
                ...
                val tapPath = aarishV6TapPath(safeX, safeY, screenW, screenH)
                dispatchPath(tapPath, 60L) { ok ->
                    if (!ok || !isSamePlaybackRun(runId)) {
                        ...
                        handler.postDelayed(
                            { runBurst(index + 1) },
                            GestureStore.aarishV6HumaniseDelay(this, GestureStore.aarishV6GetMultiTapGapMs(this))
                        )
                    }
                }
            }
        ...
        dispatchPath(buildReplayPath(), normalDuration) { completed ->
            finishOnce(completed && isSamePlaybackRun(runId))
        }
    }
