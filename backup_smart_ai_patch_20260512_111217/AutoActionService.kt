    ...
    private fun findBestSmartTarget(gesture: RecordedGesture): SmartMatch? {
        return try {
            ...
            for (root in roots) {
                ...
                while (!stack.isEmpty() && visitedNodes < 1600) {
                    visitedNodes++
                    val node = stack.removeLast()
                            ...
                            if (safeVisible(targetNode) && safeEnabled(targetNode)) {
                                ...
                                if (safeBounds(targetNode, targetBounds) &&
                                    ...
                                    if (hasSavedPercentAnchor(gesture)) {
                                        ...
                                        if (dx < 0.06f && dy < 0.06f) finalScore += 28
                                        else if (dx < 0.18f && dy < 0.18f) finalScore += 14
                                        else if (dx < 0.35f && dy < 0.35f) finalScore += 5
                                    }
                                    ...
                                    val oldBest = best
                                    if (oldBest == null ||
                                        finalScore > oldBest.score ||
                                        (finalScore == oldBest.score && targetBounds.width() * targetBounds.height() < oldBest.bounds.width() * oldBest.bounds.height())
                                    ) {
            ...
            if (finalBest.score >= threshold) finalBest else null
        } catch (_: Exception) {
            null
        }
    }
    ...
    private fun scoreNode(
        ...
        return try {
            ...
            if (hasSavedPercentAnchor(gesture)) {
                ...
                if (dx < 0.05f && dy < 0.05f) score += 52
                else if (dx < 0.16f && dy < 0.16f) score += 28
                else if (dx < 0.32f && dy < 0.32f) score += 10
                else if (!strongIdentity && (dx > 0.42f || dy > 0.42f)) score -= 48
            }
...
private fun captureTargetSnapshotInternal(
        ...
        return TargetSnapshot(
            targetText = clickText ?: touchText,
            targetDesc = clickDesc ?: touchDesc,
            targetId = clickId ?: touchId,
