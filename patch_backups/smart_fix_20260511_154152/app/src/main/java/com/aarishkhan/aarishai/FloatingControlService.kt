...
class FloatingControlService : Service() {
    ...
    private fun stylePanelButton(button: Button, bgColor: Int, fgColor: Int = Color.WHITE, minWdp: Int = 52) {
    ...
    private fun refreshPanelButtonStyles() {
        ...
        val startBg = when (btnStart.text?.toString()) {
            ...
            else -> Color.rgb(79, 70, 229)
        }
        stylePanelButton(btnStart, startBg, Color.WHITE, 62)
        if (::btnLoop.isInitialized) stylePanelButton(btnLoop, Color.rgb(30, 64, 175), Color.WHITE, 54)
        if (::btnWorkflow.isInitialized) stylePanelButton(btnWorkflow, Color.rgb(88, 28, 135), Color.WHITE, 54)
        btnClear?.let { stylePanelButton(it, Color.rgb(71, 85, 105), Color.WHITE, 48) }
        if (::btnSave.isInitialized) stylePanelButton(btnSave, Color.rgb(5, 150, 105), Color.WHITE, 56)
        if (::btnUndo.isInitialized) stylePanelButton(btnUndo, Color.rgb(15, 118, 110), Color.WHITE, 58)
        if (::btnCut.isInitialized) stylePanelButton(btnCut, Color.rgb(190, 18, 60), Color.WHITE, 48)
    }
...
private fun showLoopSettingsDialog() {
    ...
    private fun undoLastStep() {
        ...
        if (cleanGestures.isEmpty()) {
            ...
            updateUIState(if (hasOld) "PLAY" else "START", false, hasOld, true)
            restorePanelUI()
            Toast.makeText(this, "Empty/invalid recording skip ho gayi", Toast.LENGTH_SHORT).show()
            return
        }
