...
private fun stylePanelButton(button: Button, bgColor: Int, fgColor: Int = Color.WHITE, minWdp: Int = 44) {
    val radius = 15f * resources.displayMetrics.density

    button.minHeight = dp(38)
    button.minimumHeight = dp(38)
    button.minWidth = dp(minWdp)
    button.minimumWidth = dp(minWdp)
    button.setPadding(dp(7), 0, dp(7), 0)
    button.textSize = 10.8f
...
private fun refreshPanelButtonStyles() {
    ...
    val startBg = when (btnStart.text?.toString()) {
        ...
        "HIDE" -> Color.rgb(217, 119, 6)
        "+ ADD" -> Color.rgb(37, 99, 235)
        "PLAY" -> Color.rgb(5, 150, 105)
        else -> Color.rgb(99, 102, 241)
    }

    stylePanelButton(btnStart, startBg, Color.WHITE, 50)
    if (::btnLoop.isInitialized) stylePanelButton(btnLoop, Color.rgb(14, 116, 144), Color.WHITE, 42)
    if (::btnWorkflow.isInitialized) stylePanelButton(btnWorkflow, Color.rgb(124, 58, 237), Color.WHITE, 44)
    btnClear?.let { stylePanelButton(it, Color.rgb(71, 85, 105), Color.WHITE, 36) }
    if (::btnSave.isInitialized) stylePanelButton(btnSave, Color.rgb(5, 150, 105), Color.WHITE, 44)
    if (::btnUndo.isInitialized) stylePanelButton(btnUndo, Color.rgb(15, 118, 110), Color.WHITE, 46)
    if (::btnCut.isInitialized) stylePanelButton(btnCut, Color.rgb(190, 18, 60), Color.WHITE, 36)
}
    ...
    private fun showFloatingPanel() {
        val root = LinearLayout(this).apply {
            ...
            background = android.graphics.drawable.GradientDrawable(
                ...
                intArrayOf(Color.rgb(15, 23, 42), Color.rgb(30, 41, 59), Color.rgb(49, 46, 129))
            ).apply {
        ...
        label = TextView(this).apply {
            ...
            maxWidth = (72 * resources.displayMetrics.density).toInt()
            isClickable = true
...
private fun restorePanelUI() {
    ...
    btnSave.visibility = if (unsavedGestures.isNotEmpty() || isRecording) View.VISIBLE else View.GONE
    btnUndo.visibility = if (hasAnythingToUndo()) View.VISIBLE else View.GONE
    btnCut.visibility = View.VISIBLE
    label.visibility = View.VISIBLE
    btnLoop.visibility = if (hasSaved) View.VISIBLE else View.GONE

    if (::btnWorkflow.isInitialized) {
        updateWorkflowButtonUI()
        btnWorkflow.visibility = if (hasSaved) View.VISIBLE else View.GONE
    }
