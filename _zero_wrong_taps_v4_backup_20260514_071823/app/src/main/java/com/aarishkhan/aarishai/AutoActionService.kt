...
private fun findExactActionButtonAcrossWindows(gesture: RecordedGesture): SmartMatch? {
    ...
    private fun findBestSmartTarget(gesture: RecordedGesture): SmartMatch? {
        return godFindShiftedButton(gesture)
    }
...
private fun performGestureAt(
    ...
    private fun captureTargetSnapshotInternal(
        ...
        return try {
            ...
            val parent = safeParent(clickNode)
            ...
            TargetSnapshot(
                ...
                targetContextText = godSubtreeText(clickNode, 60, 700),
                targetChildText = godSubtreeText(touchedNode, 40, 500),
                targetSiblingText = godParentText(clickNode),
                targetRoleFlags = listOf(
                    if (safeClickable(clickNode)) "click" else "",
                    if (godSafeRead(false) { clickNode.isLongClickable }) "long" else "",
                    if (godSafeRead(false) { clickNode.isScrollable }) "scroll" else "",
                    if (godSafeRead(false) { clickNode.isEditable }) "edit" else "",
                    if (godSafeRead(false) { clickNode.isCheckable }) "check" else ""
                ).filter { it.isNotBlank() }.joinToString("|"),
                targetTreePath = godStableKey(clickNode),

                targetParentText = godParentText(clickNode),
                targetParentId = safeId(parent),
                targetParentClass = safeClass(parent),
                targetNodeIndex = godNodeIndexInParent(clickNode),
                targetClickable = safeClickable(clickNode),
                targetStableKey = godStableKey(clickNode),
