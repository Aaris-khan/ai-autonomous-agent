...
private fun getRealAppRootForPoint(tapX: Int?, tapY: Int?): AccessibilityNodeInfo? {
    ...
    private fun findDeepestNodeAtCoordinate(
        ...
        return try {
            ...
            while (!stack.isEmpty() && visited < 1800) {
                visited++
                val item = stack.removeLast()
                val node = item.first
                val depth = item.second
...
private fun findExactActionButtonAcrossWindows(gesture: RecordedGesture): SmartMatch? {
    ...
    private fun findBestSmartTarget(gesture: RecordedGesture): SmartMatch? {
        return try {
            ...
            while (!stack.isEmpty() && visitedNodes < 1100) {
                visitedNodes++
                val node = stack.removeLast()
                    ...
                    if (score > 0) {
                        ...
                        if (safeBounds(targetNode, targetBounds) &&
                            ...
                            if (gesture.targetWPercent > 0f && gesture.targetHPercent > 0f) {
                                ...
                                val wDiff = abs(wNow - gesture.targetWPercent)
                                val hDiff = abs(hNow - gesture.targetHPercent)
                            ...
                            if (gesture.xPercent > 0f && gesture.yPercent > 0f) {
                                ...
                                if (dx < 0.06f && dy < 0.06f) finalScore += 35
                                    ...
                                    val strongIdentity =
                                        ...
                                        !gesture.targetContextText.isNullOrBlank() ||
                                        !gesture.targetTreePath.isNullOrBlank()
                                    if (!strongIdentity) finalScore -= 20
                                }
                            }

                            val dist = kotlin.math.hypot(
                                (targetBounds.exactCenterX() - fallbackX).toDouble(),
                                (targetBounds.exactCenterY() - fallbackY).toDouble()
                            ).toFloat()

                            val candidate = SmartMatch(targetNode, Rect(targetBounds), finalScore, dist)
            ...
            val finalBest = best ?: return null
            if (finalBest.effectiveScore >= 55) finalBest else null
        } catch (_: Exception) {
            null
        }
    }
    ...
    private fun scoreNode(
        ...
        return try {
            ...
            if (!gesture.targetId.isNullOrBlank() && nodeId == gesture.targetId) score += 100
