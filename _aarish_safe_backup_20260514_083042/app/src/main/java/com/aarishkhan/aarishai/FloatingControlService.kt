...
private fun extractAndAppendGestures() {
    ...
    private fun saveRecording() {
        if (isRecording) {
            ...
            if (liveView != null) {
                extractAndAppendGestures()
                forceRemoveGreenGlassNow("save")
            } else {
                clearGreenGlassStateOnly()
                Toast.makeText(this, "⚠️ Green Glass missing tha. Saved buffer hi save hoga.", Toast.LENGTH_LONG).show()
            }
        }
        ...
        val ok = if (!GestureStore.save(this, unsavedGestures)) {
            Toast.makeText(this, "❌ Save fail hua. Recording safe nahi hui.", Toast.LENGTH_LONG).show()
            return
        }
        if (!ok) {
            updateUIState("+ ADD", true, false, true)
            restorePanelUI()
            Toast.makeText(this, "❌ Memory save fail ho gayi", Toast.LENGTH_LONG).show()
            return
        }
    ...
    private fun saveCurrentGesture() {
        ...
        val newGesture = RecordedGesture(
            ...
            targetClass = snapshot?.targetClass,
                targetParentText = snapshot?.targetParentText,
                targetParentId = snapshot?.targetParentId,
                targetParentClass = snapshot?.targetParentClass,
                targetNodeIndex = snapshot?.targetNodeIndex ?: -1,
                targetClickable = snapshot?.targetClickable ?: false,
                targetStableKey = snapshot?.targetStableKey,
