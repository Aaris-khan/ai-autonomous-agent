...
private fun stopPlaybackInternal(showToast: Boolean = true) {
    ...
    private fun safeLongClickable(node: AccessibilityNodeInfo?): Boolean =
        safeNodeRead(false) { node?.isLongClickable == true }

    private fun safeEditable(node: AccessibilityNodeInfo?): Boolean =
        safeNodeRead(false) { node?.isEditable == true }

    private fun safeScrollable(node: AccessibilityNodeInfo?): Boolean =
        safeNodeRead(false) { node?.isScrollable == true }

    private fun safeCheckable(node: AccessibilityNodeInfo?): Boolean =
        safeNodeRead(false) { node?.isCheckable == true }

    private fun safeChecked(node: AccessibilityNodeInfo?): Boolean =
        safeNodeRead(false) { node?.isChecked == true }

    private fun safeFocusable(node: AccessibilityNodeInfo?): Boolean =
        safeNodeRead(false) { node?.isFocusable == true }

    private fun safeSelected(node: AccessibilityNodeInfo?): Boolean =
        safeNodeRead(false) { node?.isSelected == true }
    ...
    private fun safeNodeVisible(node: AccessibilityNodeInfo?): Boolean = safeVisible(node)
    private fun safeNodeEnabled(node: AccessibilityNodeInfo?): Boolean = safeEnabled(node)
    private fun safeNodeClickable(node: AccessibilityNodeInfo?): Boolean = safeClickable(node)
    private fun safeChildCount(node: AccessibilityNodeInfo?): Int = safeChildCount(node)
    private fun safeChild(node: AccessibilityNodeInfo?, index: Int): AccessibilityNodeInfo? = safeChild(node, index)
    private fun safeNodeParent(node: AccessibilityNodeInfo?): AccessibilityNodeInfo? = safeParent(node)
    private fun safeNodeBounds(node: AccessibilityNodeInfo?, out: Rect): Boolean = safeBounds(node, out)
    private fun safeNodeText(node: AccessibilityNodeInfo?): String? = safeText(node)
    private fun safeNodeDesc(node: AccessibilityNodeInfo?): String? = safeDesc(node)
    private fun safeNodeId(node: AccessibilityNodeInfo?): String? = safeId(node)
    private fun safeNodeClass(node: AccessibilityNodeInfo?): String? = safeClass(node)
    ...
    private fun findExactActionButtonAcrossWindows(gesture: RecordedGesture): SmartMatch? {
        val wanted = when {
            isAarishToolbarActionLabel(gesture.targetText) -> gesture.targetText
            isAarishToolbarActionLabel(gesture.targetDesc) -> gesture.targetDesc
            else -> null
        } ?: return null
        ...
        for (root in roots) {
            ...
            while (!stack.isEmpty() && visited < 900) {
                ...
                val textMatch = safeText(node)?.trim()?.equals(wanted, ignoreCase = true) == true
                val descMatch = safeDesc(node)?.trim()?.equals(wanted, ignoreCase = true) == true
                if ((textMatch || descMatch) && safeVisible(node) && safeEnabled(node)) {
                    val target = findClickableParent(node) ?: node
                    val b = Rect()
                    if (safeBounds(target, b)) {
                        val score = 500 + identityConfidenceForNode(target, b, gesture)
                        if (best == null || score > best!!.score) best = SmartMatch(target, Rect(b), score)
                    }
                }
...
private fun getRealAppRootForPoint(tapX: Int?, tapY: Int?): AccessibilityNodeInfo? {
    ...
    fun badPackage(pkg: String): Boolean {
    ...
    fun consider(root: AccessibilityNodeInfo?, focusedBonus: Int) {
        if (root == null) return
        val pkg = safePackage(root).orEmpty()
        if (badPackage(pkg)) return
        val b = Rect()
        if (!safeBounds(root, b)) return
        var score = b.width() * b.height() + focusedBonus
        if (tapX != null && tapY != null && b.contains(tapX, tapY)) score += 2_000_000
        if (score > bestScore) {
            bestScore = score
            bestRoot = root
        }
    ...
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
        try {
            for (window in windows) {
                ...
                val bonus = if (window.isFocused || window.isActive) 1_000_000 else 0
                consider(root, bonus)
            }
        } catch (_: Exception) {}
    }

    consider(rootInActiveWindow, 500_000)
    return bestRoot
}
    private fun findClickableParent(node: AccessibilityNodeInfo?): AccessibilityNodeInfo? {
        ...
        while (cur != null && depth < 30) {
            if (safeClickable(cur)) return cur
            val next = safeParent(cur)
            ...
            while (!stack.isEmpty() && visited < 2200) {
                ...
                if (bounds.contains(x, y)) {
                    ...
                    for (i in 0 until cnt) {
                        safeChild(node, i)?.let { stack.add(Pair(it, depth + 1)) }
                    }
                }
            }
...
private fun identityConfidenceForNode(node: AccessibilityNodeInfo, bounds: Rect, gesture: RecordedGesture): Int {
    ...
    if (gesture.xPercent > 0f && gesture.yPercent > 0f) {
        ...
        if (dx < 0.06f && dy < 0.06f) score += 45
            ...
            val strongIdentity = (!gesture.targetId.isNullOrBlank() && nodeId == gesture.targetId) ||
                exactText || exactDesc || contextSim >= 0.75f || dnaSim >= 0.82f
...
private fun isAarishToolbarActionLabel(label: String?): Boolean {
...
private fun findExactActionButtonAcrossWindows(gesture: RecordedGesture): SmartMatch? {
    ...
    private fun findBestSmartTarget(gesture: RecordedGesture): SmartMatch? {
        return try {
            ...
            var visited = 0
            while (!stack.isEmpty() && visited < 4200) {
                visited++
                val node = stack.removeLast()
                val bounds = Rect()
                    ...
                    if (rawScore > 0) {
                        ...
                        if (safeVisible(targetNode) && safeEnabled(targetNode)) {
                            ...
                            if (safeBounds(targetNode, targetBounds) &&
                                ...
                                var finalScore = rawScore + if (safeClickable(targetNode)) 12 else 0

                                if (gesture.targetWPercent > 0f && gesture.targetHPercent > 0f) {
                                    val wDiff = abs(targetBounds.width() / screenW - gesture.targetWPercent)
                                    val hDiff = abs(targetBounds.height() / screenH - gesture.targetHPercent)
                                    when {
                                        wDiff < 0.04f && hDiff < 0.04f -> finalScore += 10
                                        wDiff < 0.08f && hDiff < 0.08f -> finalScore += 5
                                    }
                                }
                                ...
                                if (best == null || finalScore > best!!.score) {
                                    runnerUp = best
                                    best = candidate
                                } else if (runnerUp == null || finalScore > runnerUp!!.score) {
                                    runnerUp = candidate
                                }
                            }
                        }
            ...
            val finalBest = best ?: return null
            val finalRunner = runnerUp

            val threshold = smartMatchThreshold(finalBest, gesture)
            if (finalBest.score < threshold) return null

            if (finalRunner != null && isAmbiguousSmartMatch(finalBest, finalRunner, gesture)) {
                return null
            }
    ...
    private fun scoreNode(
        ...
        return try {
            ...
            val contextSim = tokenSimilarity(
                gesture.targetContextText,
                collectNodeTextLimited(contextNode, 84, 860)
            )
            val childSim = tokenSimilarity(
                gesture.targetChildText,
                collectNodeTextLimited(node, 52, 560)
            )
            val siblingSim = tokenSimilarity(
                gesture.targetSiblingText,
                collectSiblingText(contextNode, 700)
            )
            val roleSim = roleSimilarity(gesture.targetRoleFlags, roleFlagsOf(contextNode))
            val dnaSim = dnaSimilarity(gesture.targetTreePath, extractTreePathDNA(contextNode))

            when {
                contextSim >= 0.85f -> score += 65
                contextSim >= 0.60f -> score += 35
            }
            when {
                childSim >= 0.85f -> score += 35
                childSim >= 0.60f -> score += 18
            }
            when {
                siblingSim >= 0.85f -> score += 30
                siblingSim >= 0.60f -> score += 14
            }
            if (roleSim >= 0.75f) score += 15
            when {
                dnaSim >= 0.90f -> score += 55
                dnaSim >= 0.70f -> score += 28
                dnaSim >= 0.50f -> score += 12
            }
            ...
            val exactId = !gesture.targetId.isNullOrBlank() && nodeId == gesture.targetId
            val exactText = !gesture.targetText.isNullOrBlank() &&
                nodeText?.equals(gesture.targetText, ignoreCase = true) == true
            val exactDesc = !gesture.targetDesc.isNullOrBlank() &&
                nodeDesc?.equals(gesture.targetDesc, ignoreCase = true) == true

            if (exactId) score += 100
            if (exactText) score += 90
            if (exactDesc) score += 90
            ...
            if (!gesture.targetClass.isNullOrBlank() && !nodeClass.isNullOrBlank()) {
                val savedCls = gesture.targetClass.substringAfterLast('.').lowercase()
                val nowCls = nodeClass.substringAfterLast('.').lowercase()
                if (savedCls == nowCls ||
                    (savedCls.contains("button") && nowCls.contains("button")) ||
                    (savedCls.contains("text") && nowCls.contains("text")) ||
                    (savedCls.contains("image") && nowCls.contains("image"))
                ) score += 18
            }
            ...
            if (gesture.targetWPercent > 0f && gesture.targetHPercent > 0f) {
                ...
                val hDiff = abs(bounds.height() / screenH - gesture.targetHPercent)
                when {
                    wDiff < 0.04f && hDiff < 0.04f -> score += 20
                    wDiff < 0.08f && hDiff < 0.08f -> score += 10
                }
            }
            ...
            if (gesture.xPercent > 0f && gesture.yPercent > 0f) {
                ...
                when {
                    dx < 0.05f && dy < 0.05f -> score += 60
                    dx < 0.15f && dy < 0.15f -> score += 30
                    dx < 0.28f && dy < 0.28f -> score += 10
                    dx > 0.40f || dy > 0.40f -> {
                        val strongIdentity = exactId || exactText || exactDesc ||
                            contextSim >= 0.70f || childSim >= 0.70f ||
                            siblingSim >= 0.70f || dnaSim >= 0.80f
                        if (!strongIdentity) score -= 45
                    }
                }
            }

            val areaRatio = (bounds.width().toFloat() * bounds.height()) / (screenW * screenH).coerceAtLeast(1f)
            val exactHit = exactId || exactText || exactDesc
            if (areaRatio > 0.72f && !exactHit) score -= 28
            if (areaRatio > 0.42f && !exactHit && contextSim < 0.62f) score -= 14
...
private fun performGestureAt(
    ...
    private fun safeVisible(n: AccessibilityNodeInfo?) = safeNodeRead(false) { n?.isVisibleToUser == true }
    private fun safeEnabled(n: AccessibilityNodeInfo?) = safeNodeRead(false) { n?.isEnabled == true }
    private fun safeClickable(n: AccessibilityNodeInfo?) = safeNodeRead(false) { n?.isClickable == true }
    private fun safeChildCount(n: AccessibilityNodeInfo?) = safeNodeRead(0) { n?.childCount ?: 0 }
    private fun safeChild(n: AccessibilityNodeInfo?, i: Int): AccessibilityNodeInfo? = safeNodeRead(null) { n?.getChild(i) }
    private fun safeParent(n: AccessibilityNodeInfo?): AccessibilityNodeInfo? = safeNodeRead(null) { n?.parent }
    private fun safeBounds(n: AccessibilityNodeInfo?, out: Rect): Boolean = safeNodeRead(false) {
        n?.getBoundsInScreen(out)
        n != null
    }
    private fun safeText(n: AccessibilityNodeInfo?): String? = safeNodeRead(null) { n?.text?.toString() }
    private fun safeDesc(n: AccessibilityNodeInfo?): String? = safeNodeRead(null) { n?.contentDescription?.toString() }
    private fun safeId(n: AccessibilityNodeInfo?): String? = safeNodeRead(null) { n?.viewIdResourceName }
    private fun safeClass(n: AccessibilityNodeInfo?): String? = safeNodeRead(null) { n?.className?.toString() }
    private fun safePackage(n: AccessibilityNodeInfo?): String? = safeNodeRead(null) { n?.packageName?.toString() }
    ...
    private fun tokenSimilarity(saved: String?, current: String?): Float {
        ...
        var hits = 0
        for (t in tokens) if (b.contains(t)) hits++
    ...
    private fun roleSimilarity(savedFlags: String?, currentFlags: String?): Float {
        ...
        var hits = 0
        for (f in saved) if (currentFlags.contains(f)) hits++
    ...
    private fun captureTargetSnapshotInternal(
        ...
        val touchNode = findDeepestNodeAtCoordinate(root, x, y) ?: return null
        val clickNode = findClickableParent(touchNode) ?: touchNode
        val parentNode = safeParent(clickNode)
        val grandNode = safeParent(parentNode)

        val clickBounds = Rect()
        if (!safeBounds(clickNode, clickBounds) || clickBounds.isEmpty) return null

        val safeW = screenW.coerceAtLeast(1f)
        val safeH = screenH.coerceAtLeast(1f)

        fun clean(v: String?) = v?.replace(Regex("\\s+"), " ")?.trim()?.take(180)?.takeIf { it.isNotBlank() }
        fun first(vararg vs: String?) = vs.firstNotNullOfOrNull { clean(it) }

        val clickText = clean(safeText(clickNode))
        val touchText = clean(safeText(touchNode))
        val clickDesc = clean(safeDesc(clickNode))
        val touchDesc = clean(safeDesc(touchNode))
        val clickId = clean(safeId(clickNode))
        val touchId = clean(safeId(touchNode))

        val contextChars = 860
        val childChars = 560
        val siblingChars = 700

        val clickContext = collectNodeTextLimited(clickNode, 84, contextChars)
        val touchContext = collectNodeTextLimited(touchNode, 52, childChars)
        val siblingContext = collectSiblingText(clickNode, siblingChars)
        val parentContext = collectNodeTextLimited(parentNode, 50, childChars)
        val grandContext = collectNodeTextLimited(grandNode, 34, 360)

        val mergedContext = listOf(
            ownLabelOf(clickNode),
            ownLabelOf(touchNode),
            clickContext,
            touchContext,
            parentContext,
            grandContext
        ).filter { it.isNotBlank() }.joinToString(" | ").take(contextChars)

        val xPct = (x / safeW).coerceIn(0f, 1f)
        val yPct = (y / safeH).coerceIn(0f, 1f)
        val wPct = (clickBounds.width() / safeW).coerceIn(0f, 1f)
        val hPct = (clickBounds.height() / safeH).coerceIn(0f, 1f)
        val inXPct = if (clickBounds.width() > 0) {
            ((x - clickBounds.left).toFloat() / clickBounds.width()).coerceIn(0f, 1f)
        } else 0.5f
        val inYPct = if (clickBounds.height() > 0) {
            ((y - clickBounds.top).toFloat() / clickBounds.height()).coerceIn(0f, 1f)
        } else 0.5f

        return TargetSnapshot(
            targetText = first(clickText, touchText, clickDesc, touchDesc, idTail(clickId), idTail(touchId)),
            targetDesc = first(clickDesc, touchDesc, clickText, touchText),
            targetId = first(clickId, touchId),
            targetClass = first(safeClass(clickNode), safeClass(touchNode)),
            targetContextText = mergedContext,
            targetChildText = listOf(touchContext, clickContext).filter { it.isNotBlank() }.joinToString(" | ").take(childChars),
            targetSiblingText = siblingContext.take(siblingChars),
            ...
            targetWPercent = wPct,
            targetHPercent = hPct,
            insideXPercent = inXPct,
            insideYPercent = inYPct,
        ...
        private fun findDeepestNodeAtCoordinate(
            ...
            var bestScore = Int.MIN_VALUE
            var visited = 0
            while (!stack.isEmpty() && visited < 1400) {
                visited++
                val item = stack.removeLast()
                val node = item.first
                val depth = item.second
                ...
                if (bounds.contains(x, y)) {
                    ...
                    val smallAreaBonus = 500000 / area
    ...
    private fun dispatchHybridGesturePath(
        ...
        if (!isSamePlaybackRun(runId) || snakeSynergyGuardActive()) {
            onDone(false)
            return
        }
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.N) {
        ...
        fun finishOnce(ok: Boolean) {
            if (delivered.compareAndSet(false, true)) onDone(ok)
        }

        fun tinyPath() = Path().apply {
            moveTo(safeX, safeY)
            lineTo((safeX + 1f).coerceIn(2f, screenW), (safeY + 1f).coerceIn(2f, screenH))
        }

        fun dispatch(builder: GestureDescription.Builder, done: (Boolean) -> Unit) {
            val gesture = try {
                builder.build()
            } catch (_: Exception) {
                done(false)
        ...
        if (isDoubleTap) {
            val tap1 = GestureDescription.Builder().apply {
                addStroke(GestureDescription.StrokeDescription(tinyPath(), 0L, 90L))
            }
            dispatch(tap1) { ok1 ->
                if (!ok1 || !isSamePlaybackRun(runId)) {
                    finishOnce(false)
                    return@dispatch
                }
                handler.postDelayed({
                    ...
                    val tap2 = GestureDescription.Builder().apply {
                        addStroke(GestureDescription.StrokeDescription(tinyPath(), 0L, 90L))
                    }
                    dispatch(tap2) { ok2 ->
                        finishOnce(ok2 && isCurrentCallbackRun(runId))
                    }
                }, 120L)
            }
            return
        ...
        if (first != null && points.size > 1) {
            for (i in 1 until points.size) {
                val p = points[i]
                path.lineTo(
                    (startX + (p.x - first.x)).coerceIn(2f, screenW),
                    (startY + (p.y - first.y)).coerceIn(2f, screenH)
                )
                hasLine = true
            }
        }
        ...
        val stroke = GestureDescription.Builder().apply {
            addStroke(GestureDescription.StrokeDescription(path, 0L, holdDuration))
        }

        dispatch(stroke) { ok ->
            finishOnce(ok && isCurrentCallbackRun(runId))
        }
    }
            ...
            return try {
                ...
                for (root in roots) {
                    ...
                    while (!stack.isEmpty() && visited < 1600) {
                        ...
                        if (hasSelection || safeSelected(node)) return true
                        ...
                        if (safeVisible(node) && (safeClickable(node) || safeFocusable(node)) && markers.any { label.contains(it.lowercase()) }) return true
                        val count = safeChildCount(node)
                        for (i in 0 until count) {
                            val child = safeChild(node, i)
                            if (child != null) stack.add(child)
